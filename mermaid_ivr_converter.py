...
class ProductionIVRConverter:
    ...
    def convert_mermaid_to_ivr(self, mermaid_code: str) -> Tuple[List[Dict[str, Any]], List[str]]:
        """Main conversion method that app.py expects"""
        notes = []
        
        try:
            print(f"🚀 Starting enhanced conversion with {len(self.voice_files)} voice files...")
            notes.append(f"Loaded {len(self.voice_files)} voice files for conversion")
            
            # Verify ARCOS integration
            arcos_count = len([vf for vf in self.voice_files if vf.company.lower() == 'arcos'])
            if arcos_count > 0:
                notes.append(f"✅ ARCOS integration: {arcos_count} voice files loaded")
                print(f"✅ ARCOS integration: {arcos_count} voice files")
            else:
                notes.append("⚠️ No ARCOS files found - using enhanced fallback")
            
            # Parse the Mermaid diagram
            ivr_nodes = self.parse_mermaid_diagram(mermaid_code)
            
            # Add standard termination nodes if not present
            labels = [node.get('label', '') for node in ivr_nodes]
            
            if 'Problems' not in labels:
                ivr_nodes.append({
                    'label': 'Problems',
            ...
            for node in ivr_nodes:
                if 'playPrompt' in node:
                    prompts = node['playPrompt'] if isinstance(node['playPrompt'], list) else [node['playPrompt']]
                    for prompt in prompts:
                        total_segments += 1
                        if not str(prompt).startswith('NEW_VOICE_NEEDED'):
                            matched_segments += 1
            
            if total_segments > 0:
                match_rate = (matched_segments / total_segments) * 100
                notes.append(f"📊 Voice match rate: {match_rate:.1f}% ({matched_segments}/{total_segments})")
                print(f"📊 Voice match rate: {match_rate:.1f}%")
