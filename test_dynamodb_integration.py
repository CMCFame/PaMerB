"""
Test script for DynamoDB integration in PaMerB IVR converter
"""
...
def test_voice_file_loading():
    ...
    try:
        ...
        if voice_files:
            print(f"Successfully loaded {len(voice_files)} voice files")
            
            # Show sample records
            print("\nSample voice file records:")
            for i, record in enumerate(voice_files[:5]):
            ...
            print("No voice files found in database")
            return False
            
    except Exception as e:
        print(f"Error loading voice files: {e}")
        return False

def test_converter_integration():
    """Test the IVR converter with DynamoDB integration"""
    print("\n🔄 Testing IVR converter with DynamoDB...")
    ...
    try:
        ...
        if ivr_flow and js_output:
            print(f"✅ Conversion successful with DynamoDB!")
